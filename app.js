...
async function setupMenu() {
  ...
  function renderCreationsGrid() {
      ...
      // Show all experiences, grouped by "Your Creations" and "Others' Creations"
      const allExps = Object.values(experiences || {});
      const myCreations = allExps.filter(exp => exp.creatorId === room.clientId);
      const others = allExps.filter(exp => exp.creatorId !== room.clientId);

      if (allExps.length === 0) {
          creationsGrid.innerHTML = `<p>No experiences found. Click "Create New" to get started!</p>`;
          return;
      }

      // Helper to render a card
      function makeCard(exp, isMine) {
          ...
          card.innerHTML = `
            ...
            <div class="experience-info"><h3>${escapeHtml(exp.title)}</h3><p style="margin-top:6px;font-size:12px;color:#bbb;">${escapeHtml(exp.desc || '')}</p><small style="color:#9aa">${isMine ? 'You' : (escapeHtml(exp.creator || exp.creatorId || 'Unknown'))}</small></div>
            <div class="experience-actions">
              ${isMine ? '<button class="edit-button">EDIT</button>' : ''}
      ...
      // Render "Your Experiences" first
      if (myCreations.length > 0) {
          const header = document.createElement('div');
          header.style.gridColumn = '1 / -1';
          header.style.color = '#fff';
          header.style.marginBottom = '8px';
          header.innerHTML = '<strong>Your Experiences</strong>';
          creationsGrid.appendChild(header);

          myCreations.forEach(exp => creationsGrid.appendChild(makeCard(exp, true)));
      }
      ...
      if (others.length > 0) {
          ...
          header.innerHTML = "<strong>Others' Creations</strong>";
          creationsGrid.appendChild(header);

          others.forEach(exp => creationsGrid.appendChild(makeCard(exp, false)));
      }
  }
